...
class Client(Protocol):
    ...
    def send(self, cmd, timeout=None): # Call a RPC method on the server. This is used for connect, createStream, publish, etc. 
        ...
        logger.debug('Client.send cmd=%r name=%r args=%r msg=%r', cmd, cmd.name, cmd.args, cmd.toMessage())
    ...
    def messageReceived(self, msg): # invoked by base class framework to handle a receive message.
        ...
        else: logger.debug('ignoring stream message for streamId=%r', msg.streamId)
...
class NetStream(object):
    ...
    def create(self, nc, timeout=None):
        ...
        logger.debug('createStream result=%r fault=%r', result, fault)
        if result:
    ...
    def close(self):
        ...
        msg = yield self.stream.recv()
        logger.debug('closeStream response=%r', msg)
    ...
    def send(self, cmd):
        ...
        logger.debug('Stream.send cmd=%r name=%r args=%r msg=%r', cmd, cmd.name, cmd.args, msg)
...
class RTMPReader(Resource): # connect to RTMP URL and play the stream identified by id in URL.
    ...
    def run(self):
        try:
            ...
            logger.debug('RTMPReader.run() timedout')
...
def _copy_loop(filename, ns, enableAudio, enableVideo):
    ...
    try:
        while True:
            ...
            if not msg: 
                logger.debug('Reached end, re-opening the file %r', filename)
                reader.close()
                reader = yield FLVReader().open(filename)
...
def connect(url, params=None, timeout=10, duration=60, publishStream='publish', playStream='play', publishFile=None, enableAudio=True, enableVideo=True):
    ...
    '''
    logger.debug('connect url=%r params=%r timeout=%r duration=%r publishStream=%r playStream=%r publishFile=%r enableAudio=%r enableVideo=%r', url, params, timeout, duration, publishStream, playStream, publishFile, enableAudio, enableVideo)
    ...
    try: # if the remote side terminates before duration, 
        logger.debug('timeout=%r', duration)
        yield nc.client.close_queue.get(timeout=duration)
        logger.debug('received connection close')
        if publishFile and publishStream: gen1.close()
        if playFile and playStream: gen2.close()
    except (multitask.Timeout, GeneratorExit): # else wait until duration
        logger.debug('duration completed, connect closing')
        if publishFile and publishStream: gen1.close()
        if playFile and playStream: gen2.close()
        yield nc.close()
...
_usage = '''usage: python rtmpclient.py [-d] src dest
  ...
  This software depends on Python 2.7 (won't work with 2.4 or 3.0)'''
