        ...
        while True:
            ...
            if len(data) < header.size: # we don't have all data
                ...
                if hdr.type == Message.AGGREGATE:
                    ...
                    aggdata = data;
                    while len(aggdata) > 0:
                        # type=1B, size=3B, time=4B, streamId=4B, data=size bytes, backPointer=4B, value == size
...
class Client(Protocol):
    ...
    def messageReceived(self, msg):
        if (msg.type == Message.RPC or msg.type == Message.RPC3) and msg.streamId == 0:
            ...
            # logger.debug('rtmp.Client.messageReceived cmd=%r', cmd)
            if cmd.name == 'connect':
