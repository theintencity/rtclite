...
iv_null   = lambda: [0 for i in range(16)]
iv_random = lambda: [ord(random.randint(0, 255)) for i in range(16)]

def encrypt(key, data, iv, mode=CBC):
    key, keysize = list(map(ord, key)), len(key)
...
def decrypt(key, data, iv, mode=CBC):
    key, data, keysize = list(map(ord, key)), list(map(ord, data)), len(key)
    assert keysize in (16, 24, 32), 'invalid key size: %s' % keysize
...
def _core(word, iteration): # core key schedule: rotate 32-bit word 8 bits to left, apply S-box on all 4 parts and XOR the rcon output with first part
    word = word[1:] + word[:1]
    for i in range(4): word[i] = _sbox[word[i]]
...
def _expandKey(key, size, expandedKeySize): # Rijndael's key expansion: expands an 128,192,256 key into an 176,208,240 bytes key
    ...
    for j in range(size): expandedKey[j] = key[j] # set the 16, 24, 32 bytes of the expanded key to the input key
    currentSize += size
    while currentSize < expandedKeySize:
        ...
        if size == SIZE_256 and ((currentSize % size) == 16): # For 256-bit keys, we add an extra sbox to the calculation
            for l in range(4): t[l] = _sbox[t[l]]
        for m in range(4): # We XOR t with the four-byte block 16,24,32 bytes before the new expanded key.  This becomes the next four bytes in the expanded key.
...
def _aes_block(iput, key, size, isInv): # encrypt 128-bit input block against the given key of given size
    ...
    block = [iput[i+4*j] for i in range(4) for j in range(4)]
...
def _encrypt(stringIn, mode, key, size, IV):
    ...
    if stringIn != None:
        for start in range(0, len(stringIn), 16):
            plaintext = list(map(ord, stringIn[start:start+16]))
            if len(plaintext) < 16: plaintext += [0]*(16-len(plaintext))
            if mode == CFB:
                output = _aes_block(IV if firstRound else iput, key, size, False)
                firstRound = False
                # TODO: verify the following
                ciphertext = [(0 if len(plaintext)-1 < i else plaintext[0]) ^ (0 if len(output)-1 < i else output[i]) for i in range(16)]
                cipherOut += [ciphertext[k] for k in range(end-start)]
                iput = ciphertext if mode == CFB else output
            elif mode == CBC:
                iput = [plaintext[i] ^ IV[i] for i in range(16)] if firstRound else [plaintext[i] ^ ciphertext[i] for i in range(16)]
                firstRound = False
...
def _decrypt(cipherIn, originalsize, mode, key, size, IV):
    ...
    if cipherIn != None:
        for start in range(0, len(cipherIn), 16):
            ...
            if mode == CFB or mode == OFB:
                ...
                firstRound = False
                # TODO: verify the following
                plaintext = [(0 if len(output)-1 < i else output[0]) ^ (0 if len(ciphertext)-1 < i else ciphertext[i]) for i in range(16)]
                stringOut += [plaintext[k] for k in range(end-start)]
                iput = ciphertext if mode == CFB else output
            elif mode == CBC:
                output = _aes_block(ciphertext, key, size, True)
                plaintext = [IV[i] ^ output[i] for i in range(16)] if firstRound else [iput[i] ^ output[i] for i in range(16)]
                firstRound = False
                end1 = originalsize if originalsize is not None and originalsize < end else end
                stringOut += [plaintext[k] for k in range(end1-start)]
                iput = ciphertext
    return ''.join(map(chr, stringOut))

def _test(debug=False, mode=CBC, dataSize=1000, keySize=16, repeat=100):
    cleartext = ''.join([chr(random.randint(0, 255)) for i in range(dataSize)])
    if debug: print('cleartext=%r'%(cleartext,))
    for i in range(repeat):
        cypherkey, iv = [random.randint(1,255) for i in range(keySize)], [0 for i in range(keySize)]
