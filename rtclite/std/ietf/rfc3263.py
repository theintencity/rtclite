...
def resolve(uri, supported=('udp', 'tcp', 'tls'), secproto=('tls',)):
    ...
    if transport: transports = (transport,) if transport in supported else () # only the given transport is used
        ...
        if not naptr: # do not use "else", because naptr may be cleared in "if"
            srv = filter(lambda r: r[1], map(lambda p: (_rxproto.get(p, ''), _query(('%s.%s'%(p, target), T_SRV))), map(lambda t: _xproto[t], supported)))
    ...
    if numeric: result = map(lambda t: (target, port or _proto[t][1], t), transports)
    elif port: result = sum(map(lambda t: map(lambda r: (r['RDATA'], port, t), _query((target, T_A))), transports), [])
    else:
        service = None
        ...
        if not srv:
            srv = filter(lambda y: y[1], map(lambda s: (_rproto[s[3].lower()], _query((s[0], T_SRV))), service)) if service else []
        if srv:
            ...
            result = sum(map(lambda x: map(lambda y: (y['RDATA'], x[1], x[2]), (_query((x[0], T_A)) or [])), map(lambda r: (r[0], r[3], r[4]), out)), [])
    return result or map(lambda x: (x[0], port or _proto[x[1]][1], x[1]), sum(map(lambda b: map(lambda a: (a, b), map(lambda x: x['RDATA'], _query((target, T_A)))), transports), [])) # finally do A record on target, if nothing else worked
