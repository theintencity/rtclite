...
import time, sys, re, socket, select, base64, traceback, random, logging
...
class JID(str):
    ...
    ... except ValueError as e: print('exception', e)
...
class Stanza(XML):
    ...
    def __init__(self, value=None, **kwargs):
        ...
        for k,v in kwargs.items(): self.__setattr__(k, str(v)) #should include tag, to, frm, type, timestamp, xmlns
    ...
    def properties():
        ...
        return set([y.xmlns for y in [x for x in self() if x.xmlns is not None]])
...
class Connection(object):
    ...
    def message(self, filter=None):
        ...
        response = yield self.get(lambda x: not x or x.tag == 'message' and (not filter or list(filter(x))))
    ...
    def presence(self, filter=None):
        ...
        response = yield self.get(lambda x: not x or x.tag == 'presence' and (not filter or list(filter(x))))
        respond(response and Stanza(response))
    ...
    def _resolve(self, server, service='xmpp-client', protocol='tcp'): # resolve using DNS SRV for _xmpp-client._tcp.<server> and return (host, port)
        return [(y[0], y[3]) for y in sorted([(x['RDATA']['DOMAIN'].lower(), x['RDATA']['PRIORITY'], x['RDATA']['WEIGHT'], x['RDATA']['PORT']) for x in location_query(('_%s._%s.%s'%(service, protocol, server[0]), T_SRV))], lambda a,b: a[1]-b[1])] or [server]
...
class StreamOut(object):
    ...
    def put(self, data, **kwargs):
        ...
        except Exception as e: logger.exception('send error')
...
class StreamIn(parser):
    ...
    def _put(self, msg, **kwargs):
        if isinstance(self._handler, Dispatcher): self._handler.dispatch(msg) # trigger the event first
...
def authenticate(self):
    ...
    if not challenge or challenge.tag == 'failure' or challenge.xmlns != 'urn:ietf:params:xml:ns:xmpp-sasl':
        ...
        data = dict([(y[0], _unquote(y[2])) for y in [x.partition('=') for x in re.findall('(\w+=(?:"[^"]+")|(?:[^,]+))', data)]])

        if 'auth' not in list(map(str.strip, data.get('qop','').split(','))):
        ...
        res['cnonce'] = ''.join([hex(int(random.random()*65536*4096))[2:] for i in range(7)])
        ...
        data = base64.decodestring(challenge.cdata)
        data = dict([(y[0], y[2][1:-1] if y[2][:1]=='"'==y[2][-1:] else y[2]) for y in [x.partition('=') for x in re.findall('(\w+=(?:"[^"]+")|(?:[^,]+))', data)]])
