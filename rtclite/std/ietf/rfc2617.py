...
import random, hashlib, base64, time
...
def createAuthenticate(authMethod='Digest', **kwargs):
    ...
    if authMethod.lower() == 'basic':
        ...
        now = time.time(); nonce = kwargs.get('nonce', base64.b64encode('%d %s'%(now, hashlib.md5('%d:%d'%(now, id(createAuthenticate))))))
        default = dict(realm='', domain='', opaque='', stale='FALSE', algorithm='MD5', qop='auth', nonce=nonce)
        kv = [(x, kwargs.get(x, default[x])) for x in predef] + [x for x in list(kwargs.items()) if x[0] not in predef] # put predef attributes in order before non predef attributes
        return 'Digest ' + ', '.join(['%s=%s'%(y[0], _quote(y[1]) if y[0] not in unquoted else y[1]) for y in kv])
...
def createAuthorization(challenge, username, password, uri=None, method=None, entityBody=None, context=None):
    ...
    # @implements RFC2617 P6L46-P7L5
        ...
        if rest:
            ...
            arr = []
        for n,v in [x.strip().split('=') for x in arr]:
        ...
        for y in [x for x in ['username', 'realm', 'nonce', 'opaque', 'algorithm'] if x in ch]:
            cr[y] = ch[y]
        cr['uri']        = uri
        ...
        if 'qop' in ch:
            if context and 'cnonce' in context:
                ...
                cnonce, nc = H(str(random.randint(0, 2**31))), 1
            if context:
        ...
        items = sorted([x for x in cr if x not in ['name', 'authMethod', 'value', 'httpMethod', 'entityBody', 'password']])
        return authMethod + ' ' + ','.join(['%s=%s'%(y, (cr[y] if y == 'qop' or y == 'nc' else _quote(cr[y]))) for y in items])
    else:
        raise ValueError('Invalid auth method -- ' + authMethod)


# @implements RFC2617 P10L19-P10L33
H = lambda d: hashlib.md5(d).hexdigest()
KD = lambda s, d: H(s + ':' + d)
...
def digest(cr):
    ...
    algorithm, username, realm, password, nonce, cnonce, nc, qop, httpMethod, uri, entityBody \
      = [cr[x] if x in cr else None for x in ['algorithm', 'username', 'realm', 'password', 'nonce', 'cnonce', 'nc', 'qop', 'httpMethod', 'uri', 'entityBody']]
...
def basic(cr):
    ...
    return base64.b64encode(cr['username'] + ':' + cr['password'])
