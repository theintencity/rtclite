...
import os, sys, socket, time, traceback, random, logging
from struct import pack, unpack
from ... import multitask
from .rtmp import App, Header, Message, FlashServer
from .amf import AMF0
from ...app.sip.client import User, Session, MediaSession
from ...std.ietf.rfc3550 import RTP, Network as RTPNetwork
from ...std.ietf.rfc2396 import Address
from ...std.ietf.rfc4566 import SDP, attrs as format
from ...std.ietf.rfc2833 import DTMF
from ...std.ietf.rfc3261 import Header as SIPHeader
from ...common import setlocaladdr, getlocaladdr
    
try: import audiospeex, audioop
except: audiospeex = None


logger = logging.getLogger('siprtmp')
logger_media = logging.getLogger('siprtmp-media')
...
class Context(object):
    ...
    def rtmp_register(self, login=None, passwd='', display=None, rate="wideband"):
        ...
        logger.debug('rtmp-register scheme=%r aor=%r login=%r passwd=%r display=%r', scheme, aor, login, '*'*(len(passwd) if passwd else 0), display)
        ...
        try:
            ...
            if user.password:
                logger.debug('  registering addr=%r port=%r', addr, port)
                result, reason = yield user.bind(addr, refresh=True)
                logger.debug('  registration returned %r %r', result, reason)
                if result == 'failed': 
            ...
            logger.debug('  register successful %r', self.user.address)
    ...
    def rtmp_unregister(self):
        try:
            if self.user is not None:
                logger.debug('rtmp-unregister %r', self.client and self.client.path or None)
            ...
            logger.exception('exception in unregister')
    
    def rtmp_invite(self, dest, *args):
        global agent
        try:
            logger.debug('rtmp-invite %r %r', dest, args)
            if self.user: # already a registered user exists
                if not self.session: # not already in a session, so create one
                    ...
                    logger.debug('  create media context')
                    ...
                    try:
                        ...
                        logger.debug('  session=%r reason=%r', session, reason)
                    ...
                    if session: # call connected
                        ...
                        logger.debug('sip-accepted %r', codecs) 
            ...
            logger.debug('  stopping invite generator')
        except:
            logger.exception('exception in invite')
            yield self.client.call('rejected', 'Internal server error')

    def rtmp_accept(self, *args):
        global agent
        logger.debug('rtmp-accept %r', args)
        ...
        try:
            if self.user is not None and incoming is not None:
                ...
                logger.debug('  no incoming call. ignored.')
        except:
            logger.exception('exception in rtmp_accept')
    ...
    def rtmp_reject(self, reason='Decline'):
        try:
            ...
            else: logger.debug('  no incoming call. ignored')
        except: logger.exception('exception in reject')
        
    def rtmp_bye(self):
        try:
            logger.debug('rtmp-bye')
            if self.session is None and self.outgoing is not None: # pending outgoing invite
                logger.debug('  cancel outbound invite')
                self.outgoing.close()
                self.outgoing = None
            elif self.session:
                yield self._cleanup()
        except: logger.exception('exception in bye')

    def sip_invite(self, dest):
        try:
            logger.debug('sip-invite')
            yield self.client.call('invited', str(dest), str(self.user.address))
        except: logger.exception('exception in sip_invite')
        yield
        
    def sip_cancel(self, dest):
        try: 
            logger.debug('sip-cancel')
            yield self.client.call('cancelled', str(dest), str(self.user.address))
        except: logger.exception('exception in sip_cancel')
        yield
        
    def sip_bye(self):
        try: 
            logger.debug('sip-bye')
            yield self.client.call('byed')
        except: logger.exception('exception in sip_bye')
        yield
        
    def sip_hold(self, value):
        try: 
            logger.debug('sip-hold %r', value) 
            yield self.client.call('holded', value)
        except: logger.exception('exception in sip_hold')
        yield
    ...
    def _incominghandler(self): # Handle incoming SIP messages
        try:
            ...
            while True:
                ...
                logger.debug('incominghandler %r', cmd)
        ...
        except: logger.exception('incominghandler exiting')
        self._gin = None
    ...
    def _sessionhandler(self): # Handle SIP session messages
        ...
        except: logger.exception('exception in sessionhandler')
        self._gss = None
        logger.debug('sessionhandler exiting')
    ...
    def sip_data(self, fmt, data): # handle media stream received from SIP
        try:
            ...
            logger_media.debug(' <-s pt=%r seq=%r ts=%r ssrc=%r marker=%r len=%d', p.pt, p.seq, p.ts, p.ssrc, p.marker, len(p.payload))
            if self.media:
    ...
    def rtmp_data(self, stream, message): # handle media data message received from RTMP
        try:
            logger_media.debug('f->  type=%x len=%d codec=0x%02x', message.header.type, message.size, message.data and ord(message.data[0]) or -1)
            if self.media:
                ...
                if self.session and self.media.session and messages:
                    for payload, ts, marker, fmt in messages:
                        logger_media.debug(' ->s fmt=%r %r/%r ts=%r marker=%r len=%d', fmt.pt, fmt.name, fmt.rate, ts, marker, len(payload))
                        self.media.session.send(payload=payload, ts=ts, marker=marker, fmt=fmt)
        except: logger.exception('exception in rtmp_data')
        yield
        
    def rtmp_sendDTMF(self, digit):
        try:
            logger.debug('rtmp-sendDTMF %r', digit)
            if self.media:
                messages = self.media.dtmf2rtp(digit)
                if self.session and self.media.session and messages is not None:
                    for payload, ts, marker, fmt in messages:
                        self.media.session.send(payload=payload, ts=ts, marker=marker, fmt=fmt)
        except: logger.exception('exception in rtmp_sendDTMF')
        yield
        
    def rtmp_hold(self, value):
        try:
            logger.debug('rtmp-hold %r', value)
            self.session.hold(value)
        except: logger.exception('exception in rtmp_hold')
        yield
...
class MediaContext(object):
    ...
    def rtmp2rtp(self, stream, message): # public method called by Context to transcode RTMP to RTP for media.
        if self.session: # order of following processing is important
            ...
            else: logger.debug('  ignoring in rtmp2rtp type=%r size=%r', message.header.type, message.size)
    ...
    def rtp2rtmp(self, fmt, p): # public method called by Context to transcode RTP to RTMP for media
        if str(fmt.name).lower() == str(self._flv.name).lower(): # this is a video (FLV) packet, just assemble and return to rtmp
            ...
            logger.debug('ignoring incoming DTMF touchtone')
    ...
    def dtmf2rtp(self, digit): # public method called by Context to send DTMF to RTP.
        if len(digit) != 1:
            logger.debug('  only single digit DTMF is supported in sendDTMF')
        elif not self.session or not self.session.hasType('audio'):
            logger.debug('  ignoring sendDTMF: not an active audio call')
        else:
            payload = repr(DTMF(key=digit, end=True))
            logger.debug('  sending payload %r', payload)
    ...
    def _getMediaStreams(self):
        ...
        if not self._codecs: # use the default codecs for backward compatibility
            ...
            for codec in self._codecs:
                ...
                else: logger.debug('ignoring %r, may already be added', codec)
...
#        logger.debug('  FLV sending type=%d,len=%d split seq=%d, chunks=%d', message.type, origlen, self._flv1_txseq, len(packets)) 
        self._flv1_txseq += 1
    ...
    def _rtp2rtmpFLV(self, p): # convert given RTP packet to RTMP message and play to the rtmp side.
        ...
        if magic != 'RTMP':
            logger.debug('ignoring non-RTMP packet in received video')
            return
        seq, cseq = unpack('>Ih', payload[:6])
#        logger.debug('  FLV received seq=%d cseq=%d len=%d', seq, cseq, len(payload))
        if cseq == 0: # first packet in the chunks. Initialize the rx state.
            ...
            if seq != self._flv2_rxseq or len(self._flv2_rxchunks) == 0:
                logger.debug( 'probably missed a begin packet')
                return
        ...
        if self._flv2_rxlen < 12:
            logger.debug('received data is too small %d', self._flv2_rxlen)
            return
        ...
        if msglen != len(message.data):
            logger.debug('invalid message len %d != %d', msglen, len(message.data))
            return
        return [message]

    def _rtmp2rtpH264(self, message):
#        logger.debug('f-> %r %r', len(message.data), message.data[:20])
        messages = []
        if message.data[:2] == '\x17\x00': # AVC seq
            ...
            if cfgVer == 1:
                ...
                for j in range(numPPS):
                    ...
                    data = data[lenPPS:]
#                logger.debug('avcCfg: cfgVer=%r profileIdc=%r profileCompat=%r levelIdc=%r lenSize=%r numSPS=%r numPPS=%r SPS=%r PPS=%r', cfgVer, profileIdc, profileCompat, levelIdc, lenSize, numSPS, numPPS, SPS, PPS)
...
#                    logger.debug(' ->s %r %r', len(SPS[0]), SPS[0])
                    messages.append((SPS[0], ts, marker, self._h264))
                if PPS:
#                    logger.debug(' ->s %r %r', len(PPS[0]), PPS[0])
                    ts, marker = message.time * self._h264.rate / 1000, True
            ...
            if self._h1_PPS and self._h1_SPS: # indicates that SPS/PPS are sent
                try:
                    ...
                    while data:
                        ...
                        nals.append(nalData)
#                    logger.debug('      nals count=%r types=%r', len(nals), [(ord(x[0]) & 0x1f) for x in nals])
                    if nals:
                        remaining = nals[-1]
...
#                                logger.debug(' ->s %r %r', len(remaining), remaining[:15])
                                ...
                                while remaining:
                                    ...
                                    start = 0x00
#                                    logger.debug(' ->s %r %r', len(payload), payload[:15])
    ...
    def _rtp2rtmpH264(self, fmt, p):
            ...
            if nalType == 24: # cisco phone sends SPS/PPS in aggregated packet
                ...
                while payload:
                    ...
                    if nt == 7:
                        ...
                        logger.debug('extract SPS from aggregated %r', naldata)
                    elif nt == 8:
                        self._h2_PPS = naldata
                        logger.debug('extract PPS from aggregated %r', naldata)
        ...
        if len(self._h2_queue) >= 2 and (self._h2_queue[-1].seq != (self._h2_queue[-2].seq + 1)):
            if (self._h2_queue[-2].seq != 65535) or (self._h2_queue[-1].seq != 0):
                logger.debug('new packet does not directly follow previous: %r != %r + 1', self._h2_queue[-1].seq, self._h2_queue[-2].seq)

        if len(self._h2_queue) >= 2 and (self._h2_queue[-1].ts != self._h2_queue[-2].ts):
            logger.debug('clearing old queue since new packet has different ts: %r != %r', self._h2_queue[-1].ts, self._h2_queue[-2].ts)
        ...
        if p.marker and len(self._h2_queue) > 0:
            ...
            for q in queued:
                if q.nalType == 5 or q.nalType == 1:
                    ...
                    while payload:
                        ...
                        if nt == 5 or nt == 1: # don't handle 7 and 8 as they are already handled before
                            if not newdata:
                                nalType = nt
                                newdata = ('\x17' if nalType == 5 else '\x27') + '\x01\x00\x00\x00'
                            logger.debug('extract from aggregate type=%r len=%r', nalType, len(naldata))
                    ...
                    if ord(q.payload[1]) & 0x40: # end bit
                        ...
                        logger.debug('aggregated %r packets, len=%r, type=%r', len(pendingdata), len(remaining), nalType)
                        pendingdata[:] = []
            ...
            if self._context.play_stream is None or not PPS or not SPS or PPS and SPS and not sentSeq and nalType != 5:
                logger.debug('H264 drop until next intra')
                ...
                if payloads and nalType == 5: # send SPS/PPS                    
                    logger.debug("   SPS %r PPS %r", SPS, PPS)
    ...
    def accepting(self): # called by Context to return the selected codec after negotiation
        ...
        if not self._codecs: # do not set codecs for backward compatibility with older applications
            ...
            if audiospeex and session.hasType('audio') and not session.hasYourFormat(preferred): # if we have audiospeex transcoding module and remote doesn't have our preferred format, enable transcoding
                ...
                logger.debug('  enable transcoding between %r/%r and %r/%r', preferred.name if preferred else None, preferred.rate if preferred else 0, fmt.name if fmt else None, fmt.rate if fmt else 0)
    ...
    def _rtmp2rtpAU(self, message):
        global audiospeex
#        logger.debug('  AU received %r', message.data[0])
        ...
        if not self._au1_fmt: # no transcoding needed
            if codec == 'speex' and session.hasYourFormat(self._wideband):
                ...
                fmt = self._pcma
            else: logger.debug('ignoring codec audio type %r', first)
...
#        logger.debug('  RTMP pt=%x len=%d hdr=%r', m.header.type, m.size, m.header)
        return [m]
...
class Gateway(App):
    ...
    def onCommand(self, client, cmd, *args):
        ...
        else: logger.debug('invalid command %r', cmd)
...
if __name__ == '__main__':
    ...
    from . import rtmp
    from ...app.sip import client
    from ...std.ietf import rfc3550, rfc3261

    verbose = logging.DEBUG if options.verbose or options.verbose_all else logging.CRITICAL
    verbose_all = logging.DEBUG if options.verbose_all else logging.CRITICAL
    logger.setLevel(verbose)
    client.logger.setLevel(verbose)
    logger_media.setLevel(verbose_all)
    rtmp.logger.setLevel(verbose_all)
    rfc3550.logger.setLevel(verbose_all)
    rfc3261.logger.setLevel(verbose_all)
    
    if not audiospeex:
        logger.warning('audiospeex module not found; disabling transcoding to/from speex')
