...
#  $ export PYTHONPATH=.:/path/to/PyCrypto
#  $ python -m rtclite.vnd.adobe.rtmfp -d --no-rtmp
...
#  $ python -m rtclite.vnd.adobe.rtmfp -d --no-rtmp --middle
...
logger = logging.getLogger('rtmfp')
...
def _decode(decoder, data):
    ...
    if struct.unpack('>H', raw[4:6])[0] != _checkSum(raw[6:]):
        logger.error('decode() invalid checksum %x != %x data=%r', struct.unpack('>H', raw[4:6])[0], _checkSum(raw[6:]), raw[6:])
...
class Listener(object):
    ...
    def computeTime(self, tm):
        ...
        if self._deltaTime > tm:
            logger.debug('Listener.computeTime() time lower than deltaTime on listener %d %r %r', self.id, tm, self._deltaTime)
            self._deltaTime = tm
    ...
    def flush(self, flags=0, message=''):
        ...
        if len(data) > 0:
            ...
            for i in range(2):
                ...
                elif i > 1: logger.debug('Session.flush() socket send error on sessin %u, all data was not sent', self.id)
            if not message:
                self._writer.clear()
...
#        self._pending = [] # list of pending messages from initiator before handshake with terminator is complete
    ...
    def receiveFromTarget(self):
        try:
            while True:
                try:
                    ...
                    logger.exception('middle socket reception error')
                    return
                if remote != self.target.address:
                    logger.error('received from wrong target %r != %r', remote, self.target.address)
                    continue
                if len(data) < 12:
                    logger.error('middle from %r: invalid packet size %r', remote, len(data))
                    continue
                id = _unpackId(data)
        ...
        except: logger.exception('exception in Middle task')
...
#        self.server.send(data, self.target.address)
    ...
    def sendToTarget(self, data):
        if not self._middleAesEncrypt:
            logger.error('critical error: send to target packet impossible because the middle handshake has failed')
            return
...
#        self.server.send(data, self.target.address)
    ...
    def targetHandshakeHandler(self, type, data):
        ...
        if type == 0x70:
            ...
            if self._middleAesDecrypt:
                ...
                logger.warning('middle mode leaks: redirection request, restart with a url pertinant among the following')
                index = 0
            ...
            logger.error('unknown target handshake type 0x%02x', type)
    ...
    #        if ord(data[6]) != 0x0b:
    #            logger.error('target handshake received with a marker different than 0b')
    #            return
                        ...
                        if netGroupHeader == 0x4752:
                            ...
                            for group in self.server.groups:
                                if group.hasPeer(self.target.id):
                                    ...
                                    break
                            if not found:
                                logger.error('handshake netgroup packet between peers without corresponding group')
            elif type == 0x4C:
                self.kill()
        ...
        while remaining and ord(remaining[0]) != 0xFF:
            ...
            if type == 0x10 or type == 0x11:
                            ...
                            if flagType == 0x0b and stage == 0x01 and (marker == 0x4e and idFlow == 0x03 or marker == 0x8e and idFlow == 0x05):
                                ...
                                for group in self.server.groups:
                                    if group.hasPeer(self.target.id):
                                        ...
                                        break
                                if not found:
                                    logger.error('handshake NetGroup packet between peers without same group')
            elif type == 0x0F:
                ...
                if peerId != self.peer.id and peerId != self.middlePeer.id:
                    logger.warning('the p2p handshake target packet does not match the peerId or middlePeerId')
...
class FlashServer(rtmp.FlashServer):
    ...
    def start(self, options):
        ...
        if not self.sockUdp:
            ...
            logger.info('FlashServer.start() listening udp on %r', sock.getsockname())
...
#                logger.debug('socket.recvfrom %r\n    data=%s', remote, truncate(data))
                logger.info('<= %s:%d [%d]', remote[0], remote[1], len(data))
                #print '%r'%(data,)
                if len(data) < 12:
                    logger.error('FlashServer.serverudplistener() invalid packet of length %r', len(data))
                else:
                    ...
                    if sessionId not in self.sessions:
                        ...
                        except: logger.exception('FlashServer.serverudplistener() session exception')
        except GeneratorExit: pass # terminate
        except StopIteration: raise
        except: logger.exception('FlashServer.serverudplistener() exception')
...
#        logger.debug('socket.sendto %r\n    data=%s'%(remote, truncate(data)))
    ...
    def handshakeP2P(self, tag, address, peerIdWanted):
        ...
        if self.middle:
            logger.debug('   p2p-handshake processing middle mode target=%r', sessionWanted._target)
            if sessionWanted._target:
                ...
                logger.error('peer/peer dumped exchange impossible: no corresponding target with the session wanted')
    ...
    def manage(self):
        ...
        if self._timeLastManage < time.time() - 0.020: # more than 20ms
            logger.warning('FlashServer.manage(): process management lasted more than 20ms: %d', time.time() - self._timeLastManage)
...
if __name__ == '__main__':
    ...
    logging.basicConfig()
    logger.setLevel(logging.DEBUG if options.verbose else logging.CRITICAL)
    if options.cirrus:
        logger.debug('main() using cirrus %r', options.cirrus)
    ...
    try:
        ...
        agent.start(options)
        logger.info('%s Flash Server Starts - %s:%d', time.asctime(), options.host, options.port)
        multitask.run()
