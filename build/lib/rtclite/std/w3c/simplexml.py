...
def ustr(value):
    ...
    return r if isinstance(r, str) else str(r, 'utf-8')
...
class parser(object):
    ...
    def _StartElementHandler(self, tag, attrs):
        ...
        for n, v in [x for x in list(attrs.items()) if x[0].rfind(' ') >= 0]:
...
class XMLList(list):
    ...
    def _filter(self, func, recurse=False): # filter the elements in the sub-tree
        if recurse:
            result = XMLList()
            for x in [y for y in self if isinstance(y, XML)]:
                if func(x): result.append(x)
                res = x.children._filter(func, recurse)
                if res: result.extend(res)
            return result
        else: return XMLList([x for x in self if isinstance(x, XML) and func(x)])
    ...
    def _update(self, tag, values):
        ...
        elif isinstance(values, str): self[pos:pos] = [XML('<%s>%s</%s>'%(tag, values, tag))]
    ...
    def __contains__(self, item): # item is either XML, or tag name, or lambda function to test
        ...
        elif isinstance(item, str): return self._filter(lambda x: x.tag == item)
    ...
    def __getitem__(self, key): # key is either int, or "*", or tag name, or lambda function to test
        ...
        elif key == '*': return self._filter(lambda x: True)
        elif isinstance(key, str): return self._filter(lambda x: x.tag == key)
    ...
    def __setitem__(self, key, value): # key is either int, or tag name.
        ...
        elif key == '*': self[:] = value if isinstance(value, XMLList) else [value]; result = self
        elif isinstance(key, str): self._update(key, value); result = value
        return result

    def __delitem__(self, key): # key is same as that in __getitem__ 
        if isinstance(key, int): list.__delitem__(self, key); return None
        elif key == '*': result, self[:] = self[:], []; return result # make it empty
        elif isinstance(key, str): return self._delete(lambda x: x.tag == key)
        elif callable(key): return self._delete(key)
        else: return None
    
    # mapping related methods similar to container access for elements
    def keys(self): return set([x.tag for x in [y for y in self if isinstance(y, XML)]]) # returns a set of all tags
    ...
    def has_key(self, key): # return true if the tag exists
        for y in [x for x in self if isinstance(x, XML)]:
    ...
    def iterkeys(self): return iter(list(self.keys())) # iterator for keys
    ...
    @property
    def cdata(self): return ''.join([(x.cdata if isinstance(x, XML) else x) for x in self])
    @property
    def elems(self): return [x for x in self if isinstance(x, XML)]
    ...
    def __add__(self, other): # XMLList + XML, XMLList + XMLList, XMLList + list, XMLList + object
        ...
        else: return XMLList(self[:] + [other if isinstance(other, XML) else str(other)])
    
    def __radd__(self, other): 
        if isinstance(other, list): return XMLList(other + self[:])
        else: return XMLList([other if isinstance(other, XML) else str(other)] + self[:])
    ...
    def __iadd__(self, other): # XMLList += XML, XMLList += XMLList, XMLList += list, XMLList += object
        ...
        else: self.append(other if isinstance(other, XML) else str(other))
        return self
    
    def __isub__(self, other): # XMLList -= XML, XMLList -= XMLList, XMLList -= list, XMLList -= object
        if not isinstance(other, list): other = [other]
        self[:] = [x for x in self if x not in other]
        return self
    
    def __ixor__(self, other): # XMLList ^= XML, XMLList ^= XMLList (add only if tag is not found, else don't overwrite)
        if not isinstance(other, list): other = [other]
        self.extend([x for x in other if x.tag not in list(self.keys())])
        return self
    
    def __ior__(self, other): # XMLList |= XML, XMLList |= XMLList (overwrite if tag is found else append)
        if not isinstance(other, list): other = [other]
        overwrite = [x for x in other if x.tag in list(self.keys())]       # that needs to be overwritten
        overtags = [x.tag for x in overwrite]
        add = [x for x in other if x.tag not in list(self.keys())]         # that needs to be added
        self[:] = [x for x in self if x.tag not in overtags] # remove these for overwritting
    ...
    def __iand__(self, other): # XMLList &= XML, XMLList &= XMLList (overwrite only if tag is found, else don't append)
        ...
        overwrite = [x for x in other if x.tag in list(self.keys())]       # that needs to be overwritten
        overtags = [x.tag for x in overwrite]
        self[:] = [x for x in self if x.tag not in overtags] # remove these for overwriting
...
class XML(object):
    ...
    def __init__(self, value=None, tag='element', xmlns='', attrs={}, children=None, parent=None):
        ...
        if isinstance(self.children, str): self.children = [self.children]

    def __repr__(self):
        ns = ['xmlns="%s"'%(self.xmlns,)] if self.xmlns and (not self.parent or self.xmlns != self.parent.xmlns) else []
        attrs = ['%s="%s"'%(k, escape(ustr(v))) for k,v in self.attrs.items()]
        intag = ' '.join([self.tag] + ns + attrs)
        inner = ''.join([str(x) if isinstance(x, XML) else escape(x) for x in self.children])
        return '<%s>%s</%s>'%(intag, inner, self.tag) if inner else '<%s />'%(intag)
    
    def toprettyxml(self, encoding='UTF-8', indent='  ', count=0): # similar but not same as xml.dom.minidom's toprettyxml
        ns = ['xmlns="%s"'%(self.xmlns,)] if self.xmlns and (not self.parent or self.xmlns != self.parent.xmlns) else []
        attrs = ['%s="%s"'%(k, escape(ustr(v))) for k,v in self.attrs.items()]
        intag = ' '.join([self.tag] + ns + attrs)
        inner = ('\n' + indent*(count+1)).join([x.toprettyxml(encoding=None, indent=indent, count=count+1) if isinstance(x, XML) else escape(x.strip()) for x in self.children if not isinstance(x, str) or x.strip()])
        return ('' if encoding is None else '<?xml version="1.0"?>\n' if not encoding else '<?xml version="1.0" encoding="%s"?>\n'%(encoding,)) + \
            ('<%s>\n'%(intag,) + indent*(count+1) + inner + '\n' + indent*count + '</%s>'%(self.tag,) if len(self.elems) \
             else ('<%s>%s</%s>'%(intag, inner, self.tag) if inner else '<%s />'%(intag,)))
