...
# this module level property can be modified to '\n' to use only LF in generated SDP
lineending = '\r\n' 
...
def is_token(value):
    '''BNF includes token-char values !#$%&'*+-.0-9A-Z^_`a-z{|}~'''
    return value and not not re.match(r"[!#$%&'\*\+\-\.0-9A-Z^_`{\|}~a-z]+$", value)
        ...
        for k in [x for x in 'vosiuepcbtam' if x in self]: # order is important
            if k not in SDP._multiple: # single header
                ...
                for v in self[k]:
                    result += k + '=' + str(v) + '\r\n'
        if lineending != '\r\n':
            if lineending != '\n': raise RuntimeError('lineending must be either "\r\n" or "\n"');
            result = re.sub(r'\r\n', lineending, result)
        return result
